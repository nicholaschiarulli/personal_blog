...
const Home = ({
  ...
  const [posts, setPosts] = useState<Post[]>([]) // how does use state work
  const currentCategory = pageContext.category // what is page context
  ...
  useLayoutEffect(() => {
    ...
    filteredPostData.forEach(({ node }) => {
      const { id } = node
      const { slug } = node?.fields! // what is ! for
